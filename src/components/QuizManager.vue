  ...
  <keep-alive>
    <div class="container">
      <div class="left-column">
        ...
        <div>
          <div id="instructionsModal" class="modal" @click="closeModalOnClickOutside" ref="modalBackdrop">
        ...
        <details>
          ...
          <div>
            ...
            <input type="checkbox" id="randomize-questions" v-model="randomizeQuestions" />
          </div>
          <div>
            <label for="calculate-quiz-score">Calculate quiz score</label>
            <input type="checkbox" id="calculate-quiz-score" v-model="calculateQuizScore" />
          </div>
          <div v-if="calculateQuizScore">
            <label for="passing-grade">Passing grade</label>
            <input type="number" id="passing-grade" v-model="passingGrade" min="0" max="100" step="1" />
          </div>
          <div>
            <label for="prevent-changing-answers">Prevent changing answers after submit</label>
            <input type="checkbox" id="prevent-changing-answers" v-model="preventChangingAnswers" />
          </div>
          <div>
            <label for="end-quiz-button">End quiz button when all questions are answered</label>
        ...
        <hr />
        <div>
          <Question v-for="(question, index) in questions" :key="index" :index="index" :question="questions[index]"
            @update:question="updateQuestion(index, $event)" @delete-question="removeQuestion"
      ...
      <div class="right-column">
        <div>
          <input type="file" ref="fileInput" @change="loadQuizData" accept=".txt,.json" style="display: none" />
        ...
        <div v-for="(question, index) in questions" :key="'preview-' + index">
          <QuestionPreview :question="question" :index="index" :readyforchange="readyForChange"
...
input[type="radio"]:checked+label:before {
  background: radial-gradient(#000 0%,
      #000 40%,
      transparent 50%,
      transparent 100%);
}
